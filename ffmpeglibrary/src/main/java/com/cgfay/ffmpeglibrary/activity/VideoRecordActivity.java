...
/**
 * 使用FFmpeg进行软编码录制的测试页面
 * 备注：这里使用的相机分辨率是640x360，有些设备是不支持的，因此需要各自独立测试。
 * 另外一个问题就是对齐的问题，比如960x540P的时候编码就会出现画面不正确、噪音等等问题
 * 由于这里只是做了简单的录制流程，如果要真正商用的话，还要把对齐、缓冲区等策略添加进来比较好。
 * 录制部分的代码是从FFmpeg的muxing.c文件修改得到的，没有其他处理，还只是个Demo，不建议直接使用
 */
public class VideoRecordActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    private AspectRatioLayout mLayoutAspect;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLayoutAspect = (AspectRatioLayout) findViewById(R.id.layout_aspect);
    ...
    private void startVideoRecord() {
        mRenderHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                AVMediaRecorder.initMediaRecorder(path, mImageWidth, mImageHeight, mImageWidth, mImageHeight,
                        25, 5760000, true, 40000, 44100);
                AVMediaRecorder.startRecord();
                mRecording = true;
            }
        });
    }
    ...
    private class VideoStreamTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            if (mData != null) {
                AVMediaRecorder.encodeYUVFrame(mData);
            }
            return null;
        }
    }
    ...
    private class AudioStreamTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            if (mData != null) {
                AVMediaRecorder.encodePCMFrame(mData, mSize);
            }
            return null;
        }
    }
