...
public class FaceDetector {
    ...
    // 是否正处于人脸关键点检测过程，如果是则跳过检测，等待检测完成
    ...
    public void create() {
        ...
        if (mDetectorHandler == null) {
            mDetectorHandler = new Handler(mHandlerThread.getLooper());
        }
    }
    ...
    public void destory() {
        ...
        mDetectorHandler.removeCallbacksAndMessages(null);
        mHandlerThread.quitSafely();
        try {
            mHandlerThread.join();
        } catch (InterruptedException e) {
     ...
     * @param height
     */
    public void faceDetecting(final byte[] data, final int width, final int height) {
        if (mDetectorHandler != null) {
            if (isDetecting)
                return;
            isDetecting = true;
            mDetectorHandler.post(new Runnable() {
                @Override
                public void run() {
                    onfaceDetecting(data, width, height);
                }
            });
        }
    }
    ...
    /**
     * 人脸检测 TODO: Face++ 官方Demo中，720P检测的时间不到20ms，这里的写法似乎存在问题
     * @param data
     * @param width
     * @param height
     */
    public void onfaceDetecting(byte[] data, int width, int height) {
        if (!isAvailable) {
            return;
        }
        // 配置旋转角度
        ...
        Log.d("faceDetecting", "track time = " + algorithmTime);
        if (faces != null) {
