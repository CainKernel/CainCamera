 ...
 * @date 2019/6/30
 */
public final class JAVVideoRecorder implements Runnable, JAVVideoEncoder.OnEncodingListener {

    private static final String TAG = "AVVideoRecorder";
    ...
    // 视频编码器
    private JAVVideoEncoder mVideoEncoder;
    ...
    public void startRecord(VideoParams params) {
        if (VERBOSE) {
            Log.d(TAG, "JAVVideoRecorder: startRecord()");
        }
        synchronized (mReadyFence) {
            if (mRunning) {
                Log.w(TAG, "JAVVideoRecorder thread already running");
                return;
            }
            mRunning = true;
            new Thread(this, "JAVVideoRecorder").start();
            while (!mReady) {
                try {
                    mReadyFence.wait();
    ...
    private static class RecordHandler extends Handler {
        ...
        private WeakReference<JAVVideoRecorder> mWeakRecorder;

        public RecordHandler(JAVVideoRecorder encoder) {
            mWeakRecorder = new WeakReference<JAVVideoRecorder>(encoder);
        }
        ...
        public void handleMessage(Message inputMessage) {
            ...
            JAVVideoRecorder encoder = mWeakRecorder.get();
            if (encoder == null) {
    ...
    private void onStartRecord(@NonNull VideoParams params) {
        ...
        try {
            mVideoEncoder = new JAVVideoEncoder(params, this);
