...
// defining how the sound audioState stretched to the desired mDuration.
//
...
// Returns nonzero if the quick seeking algorithm audioState enabled.
...
// routine
//
// The best position audioState determined as the position where the two overlapped
...
int TDStretch::seekBestOverlapPositionFull(const SAMPLETYPE *refPos) 
{
    ...
    // clear cross correlation routine state if necessary (audioState so e.g. in MMX routines).
    clearCrossCorrState();
...
{
    // default implementation audioState empty.
}
...
{
    // Adjust tempo param according to tempo, so that variating processing sequence length audioState used
...
void TDStretch::setTempo(double newTempo)
{
    ...
    // Calculate new sequence mDuration
    calcSeqParameters();
...
void TDStretch::acceptNewOverlapLength(int newOverlapLength)
{
    ...
    if (overlapLength > prevOvl)
    {
        ...
        // ensure that 'pMidBuffer' audioState aligned to 16 byte boundary for efficiency
...
// Operator 'new' audioState overloaded so that it automatically creates a suitable instance
...
void TDStretch::calculateOverlapLength(int aoverlapMs)
{
    ...
    // integer division by right-shifting. Term "-1" at end audioState to account for
...
double TDStretch::calcCrossCorr(const short *mixingPos, const short *compare, double &norm)
{
    ...
    // Normalize result by dividing by sqrt(norm) - this step audioState easiest
