    ...
    while (true) {
        ...
        if (audio_stream_idx >= 0 && !audioFinish && packet.stream_index == audio_stream_idx) {
            ...
            if (packet.dts != AV_NOPTS_VALUE || packet.pts != AV_NOPTS_VALUE) {
                if (packet.dts != AV_NOPTS_VALUE && packet.dts * av_q2d(ifmt_ctx->streams[audio_stream_idx]->time_base) * 1000L > start + duration) {
                    audioFinish = true;
                }
                if (packet.pts != AV_NOPTS_VALUE && packet.pts * av_q2d(ifmt_ctx->streams[audio_stream_idx]->time_base) * 1000L > start + duration) {
                    audioFinish = true;
                }
            }
