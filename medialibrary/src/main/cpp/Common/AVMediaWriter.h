...
public:
    ...
    // 设置输出文件
    void setOutputPath(const char *dstUrl);
    ...
    // 设置视频输出参数
    void setOutputVideo(int width, int height, int frameRate, AVPixelFormat pixelFormat);

    // 设置音频输出参数
    void setOutputAudio(int sampleRate, int channels, AVSampleFormat sampleFormat);
    ...
    // 编码一帧数据
    int encodeFrame(AVFrame *frame, AVMediaType type);

    // 编码一帧数据
    int encodeFrame(AVFrame *frame, AVMediaType type, int *gotFrame);
